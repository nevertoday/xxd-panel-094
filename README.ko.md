<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 094 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 094이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진의 모든 경계를 따라 그릴 필요는 없습니다. Panel 094는 주체의 정체성과 감정을 남기고 복잡한 배경을 물려, 하나의 형태와 관계, 넓은 종이 여백이 이야기를 맡게 합니다.

**Panel 094**는 가는 먹선, 국부적인 먹색 면, 소량의 해칭으로 형태를 잡고 사진에서 하나의 별색을 추출합니다. 매우 넓은 네거티브 스페이스, 선택적인 손그림 장식 테두리, 고전 출판 서체를 결합해 20세기 초 동화책이나 시집 속표지 같은 고요한 한 페이지로 재구성합니다.

### 이런 경우에 적합합니다

- **주체는 남기고 소음을 덜고 싶을 때:** 정체성, 자세, 감정과 서사 관계에 필요한 단서만 유지합니다.
- **고전적인 2색 인쇄를 좋아할 때:** 검정 잉크와 사진에서 뽑은 한 가지 별색을 아이보리 종이와 미세한 판 어긋남에 결합합니다.
- **여백에 의미를 주고 싶을 때:** 주체를 작게, 고립되게, 비껴 놓아 빈 공간 자체를 시적 구조로 만듭니다.
- **유연한 납품이 필요할 때:** 상하·좌우·디자인 전용·여러 비율·4종 기기 벽지·폴더 일괄 처리에 대응합니다.

### 빠른 시작

> 이 이미지를 XXD Panel 094로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 가는 먹선 · 부분 검정 · 단일 별색 · 극대 여백 · 손그림 테두리 · 고전 출판 서체
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 오래된 동화책이나 시집 속표지처럼 맑고 절제되며 숨 쉬는 2색 삽화 |
| **대표 특징** | 가는 먹선, 국부 먹색 면, 사진 유래 별색, 극단적 여백, 장식 테두리, 고전 책 레이아웃 |
...
```text
주체·윤곽·자세·서사 관계 분석 → 복잡한 배경 삭제 → 가는 먹선·국부 먹색 면·소량 해칭으로 조형 → 사진에서 한 가지 별색 추출 → 극단적 여백·선택적 장식 테두리·고전 서체로 책 페이지 서사 완성
```

## 완성작의 특징

- 가늘고 절제된 먹선, 국부 먹색 면, 소량의 해칭으로 정체성을 지키며 사진 외곽선 필터를 쓰지 않습니다.
- 검정 잉크와 사진에서 추출해 적절히 맑힌 한 가지 별색으로 2색 인쇄 리듬을 만듭니다.
- 주체는 작고 고립되거나 비껴 놓이거나 떠 있을 수 있으며, 극단적 여백이 감정 구조가 됩니다.
- 글자는 오래된 책의 머리말, 방주, 제기처럼 선택적 장식 테두리와 종이 여백에 자연스럽게 이어집니다.
- 전면 채색, 복잡한 배경, 현대 카툰, 수채 풍경, 굵은 만화선, 디지털 그라데이션, 3D, 상업 템플릿을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-094-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 094의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-094 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-094 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-094 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-094 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-094 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-094 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 094 적용, 공통 설정은 한 번만 확정
/xxd-panel-094 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-094`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-094-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-094-prompt.zh-CN.md)
...
| **[xxd-panel-094](https://github.com/nevertoday/xxd-panel-094)** | Pen-and-ink book illustration · one extracted spot colour · extreme negative space · optional decorative border · classical publishing type |
...
| **병사급** | `xxd-panel-NNN` (현재 001–094) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
