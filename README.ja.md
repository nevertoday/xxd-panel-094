<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 094 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 094 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真はすべての輪郭をなぞる必要はありません。Panel 094 は主体の同一性と感情を残し、複雑な背景を退かせ、一つの形、一つの関係、広い紙白に物語を担わせます。

**Panel 094** は細い墨線、局部のベタ黒、少量のハッチングで形を整え、写真から一つの特色を抽出します。極端に広い余白、任意の手描き装飾枠、古典的な出版書体によって、20世紀初頭の童話集や詩集の扉絵のような静かな一頁へ再構成します。

### 向いている用途

- **主体を残してノイズを消したい：** 同一性、姿勢、感情、物語関係に必要な手掛かりだけを残します。
- **古典的な二色印刷が好き：** 黒インクと写真由来の一色を、象牙色の紙とわずかな版ずれに組み合わせます。
- **余白を意味として使いたい：** 主体を小さく、孤立させ、偏置し、空白そのものを詩的な構造にします。
- **柔軟に納品したい：** 上下・左右・デザインのみ・複数比率・四端末壁紙・フォルダ一括処理に対応します。

### すぐ試す

> この画像を XXD Panel 094 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 細墨線 · 局所黒 · 単一特色 · 極大余白 · 手描き装飾枠 · 古典出版字体
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 古い童話集や詩集の扉絵のように清潔で節度があり、呼吸する二色挿絵 |
| **識別しやすい特徴** | 細い墨線、局部のベタ黒、写真由来の特色、極端な余白、装飾枠、古典書籍レイアウト |
...
```text
被写体・輪郭・姿勢・物語関係を分析 → 複雑な背景を削除 → 細い墨線・局部のベタ黒・少量のハッチングで造形 → 写真から一色を抽出 → 極端な余白・任意の装飾枠・古典書体で書頁の物語を完成
```

## 完成作の特徴

- 細く節度ある墨線、局部のベタ黒、少量のハッチングで同一性を保ち、写真輪郭フィルターにはしません。
- 黒インクと、写真から抽出して適度に純化した一色で二色印刷のリズムを作ります。
- 主体は小さく、孤立し、偏置し、宙に置くことができ、極端な余白を感情構造として使います。
- 文字は古書の頁眉、傍注、題記のように、任意の装飾枠と紙白へ自然に接続します。
- 全面着色、複雑な背景、現代カートゥーン、水彩風景、太い漫画線、デジタルグラデーション、3D、商業テンプレートを避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-094-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 094 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-094 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-094 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-094 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-094 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 094 を適用し、共通設定は一度だけ解決
/xxd-panel-094 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-094`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-094-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-094-prompt.zh-CN.md)
...
| **兵士級** | `xxd-panel-NNN`（現在 001–094） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
