<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 094 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 094 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、姿态与叙事关系 → 删除复杂背景 → 以细墨线、局部实黑与少量排线塑形 → 从照片提炼一枚专色 → 用极大留白、可选装饰边框与古典字体完成书页叙事
```

## 成品中最容易识别的风格特点

- 纤细克制的墨线、局部实黑与少量排线维持主体身份，不使用照片描边滤镜。
- 黑墨与一枚从照片提炼并适度提纯的专色建立双色印刷节奏。
- 主体可以小、孤立、偏置或悬置；极大负空间不是空缺，而是情绪结构。
- 文字像旧书页的页眉、边注或题记，与可选装饰边框和纸白自然相连。
- 拒绝全面上色、复杂背景、现代卡通、水彩风景、粗黑漫画线、数字渐变、3D 与商业模板。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-094-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–094） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
